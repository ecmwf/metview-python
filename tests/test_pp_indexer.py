...
def test_param_info_from_fs_single_file():
    ...
    g = f["u700"]
    # p = g.param_info
    p = FieldsetDb.make_param_info(g)
    assert p.name == "u"
    assert p.scalar == True
    md = {
        "shortName": "u",
    ...
    # p = g.param_info
    p = FieldsetDb.make_param_info(g)
    # p = ParamInfo.build_from_fieldset(g)
    assert p.name == "wind"
    assert p.scalar == False
    md = {
        "shortName": "wind",
    ...
    assert p.name == "t"
    assert p.scalar == True
    # TODO: here the resulting level is different than in in the standard
    # indexer test. The difference is due to the lack of param info in the
    # pure python fieldset implementation. However, ideally we shout get the
    # same result in both implementatioms.
    md = {
        "shortName": "t",
        "paramId": 130,
        "date": 20180801,
